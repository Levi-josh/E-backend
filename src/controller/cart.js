...
const shopcart = async (req, res,next) => {
    try {
        const result1 = await users.updateOne({ 'items._id': req.params.id, 'items.progressbar._id': req.params.id1 }, { $set: { 'items.$.progressbar.$[elem].progess': true } }, { arrayFilters: [{ 'elem._id': req.params.id1 }] })
...
const checkout = async (req, res,next) => {
    try {
        ...
        const result = await users.updateOne({ 'items._id': req.params.id }, { $set: { 'items.$.date': newdate } })
        const result2 = await users.updateOne({ 'items._id': req.params.id }, { $set: { 'items.$.ordercode': code } })
        const result1 = await users.updateOne({ 'items._id': req.params.id, 'items.progressbar._id': req.params.id1 }, { $set: { 'items.$.progressbar.$[elem].progess': true } }, { arrayFilters: [{ 'elem._id': req.params.id1 }] })
...
const payment = async (req, res,next) => {
    try {
        ...
        const result = await users.updateOne({ 'items._id': req.params.id }, { $set: { 'items.$.Payment': item[0] } })
...
const shipping = async (req, res,next) => {
    try {
        ...
        const result = await users.updateOne({ 'items._id': req.params.id }, { $set: { 'items.$.shipvalue': item[0] } })
...
const complete1 = async (req, res,next) => {
    try {
        const myuser = await users.findOne({ 'items._id': req.params.id })
        const cart = myuser.items.filter(prev => prev._id == req.params.id)
        const result = await users.updateOne({ _id: req.params.id1 }, { $push: { history: cart[0] } })
...
const complete2 = async (req, res,next) => {
    try {
        const myuser = await users.findOne({ 'items._id': req.params.id })
        const cart = myuser.items.filter(prev => prev._id == req.params.id)
        const filter1 = await users.updateOne({ _id: req.params.id1 }, { $pull: { items: cart[0] } })
        console.log(filter1)
        res.status(200).json({ 'message': "completed" })
    } catch (err) {
     next(err)
...
module.exports = {
   complete1, complete2, shopcart, shipping, payment, redquan, addquan, addcart, newcart,selected,checkout,delcart,delproduct
}